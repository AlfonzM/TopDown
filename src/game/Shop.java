...
public class Shop {
	...
	public Shop() throws SlickException{
		...
		items[0] = new ShopItem(2, new Point(Game.MWIDTH/2 - 100, Game.MHEIGHT/2 - 50));
		
		do{
			id2 = r.nextInt(SkillList.list.size()-1) + 1;
		}while(id2 == id1 || checkIfExists(id2));
		
		items[1] = new ShopItem(3, new Point(Game.MWIDTH/2, Game.MHEIGHT/2 - 50));
		
		do{
			id3 = r.nextInt(SkillList.list.size()-1) + 1;
		}while(id3 == id1 || id3 == id2 || checkIfExists(id3));
		
		items[2] = new ShopItem(8, new Point(Game.MWIDTH/2 + 100, Game.MHEIGHT/2 - 50));
